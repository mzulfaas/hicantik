...
public class GLCMFeatureExtraction {
	...
	public GLCMFeatureExtraction(Bitmap image, int grayLevel) throws IOException {
		this.image = image;
	...
	private void createGrayLeveledMatrix() {
		for (int i = 0; i < image.getWidth(); i++) {
			for (int j = 0; j < image.getHeight(); j++) {
				...
				int newR = Color.red(rgb);
				int newG = Color.green(rgb);
				int newB = Color.blue(rgb);
				int newA = Color.alpha(rgb);
